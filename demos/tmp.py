...
# weighted error over mu
e_fb = np.abs(Xe_fine-X_approx_fine)
e_rb = np.abs(VT_e_fine-VT_approx_fine)


e_fb = np.sum(e_fb, axis=0) / len(xi)
e_rb = np.sum(e_rb*singular_values[:, None], axis=0) / np.sum(singular_values) /5

# plotting # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #

fig, ax = plt.subplots()
ax.plot(xi.T[0], e_fb, "r--", label="full space")
ax.plot(xi.T[0], e_rb, "g--", label="reduced space")
plt.xlabel("mu")
plt.ylabel("weighted mean error")
plt.legend()
plt.show()
