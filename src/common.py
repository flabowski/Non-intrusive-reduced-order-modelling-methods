...
def time_stepping(mesh, VQ, bcs, ds_, N, dt, parameter,
                  scheme, solver, plot_res, my_dir, density_varies=False):
    print("snapshots will be saved at: "+my_dir)
    if not os.path.exists(my_dir):
        os.makedirs(my_dir)
    problem = scheme(VQ, mesh, bcs, dt, parameter)  # navier_stokes_IPCS
    # store n_snapshots at a time
    n_snapshots = 10000  # per batch
    ...
    for n in trange(N):
        ...
        if density_varies:
            r_1 = res[2]
            density[ind] = r_1.compute_vertex_values(mesh)
        time[n] = t
        ind += 1
