...
def snapshots(file, snapshots_per_dataset):
    ...
    print(file, data["Re"], data["rho"], data["mu"], t[[0, -1]])
...
def load_snapshots(path, snapshots_per_dataset):
    ...
    n_snapshots = snapshots_per_dataset*n_datasets
    ...
    for i, file in enumerate(files):
        # s, e = i*snapshots_per_dataset, (i+1)*snapshots_per_dataset
        U_u[:, i], U_v[:, i], U_p[:, i], t[i], mu[i] = \
            snapshots(path+file, snapshots_per_dataset)
...
if __name__ == "__main__":
    ...
    plt.close("all")
    U_u, U_v, U_p, t, mu, x, y, tri = load_snapshots(path, snapshots_per_dataset)
    ...
    time_corrected, period = normalize_phase(V_full, t, mu)
    ...
    # (n_nodes, n_datasets, snapshots_per_dataset)
    x1 = mu[:, None] * np.ones((n_datasets, 3*snapshots_per_dataset))  # 9, 3*150
    x2 = np.c_[time_corrected-period[:, None],
               time_corrected,
               time_corrected+period[:, None]]  # 9, 3*150
    # for i in range(n_datasets):
    if True:
        i = 5
        s, e = i, (i+1)
        ...
        x2_validation = time_corrected[i, :]
        ...
        for j in range(3):
            for k in range(n_modes):
                d_ = V[j].numpy()[:, k].reshape(n_datasets, snapshots_per_dataset).copy()
        ...
        # V_interpolated[j][0, :]
        # V[j].numpy()[300, :n_modes]
            print("V interpolated the same as in V?") # yep
            for tmp_i in range(50):
                print(np.allclose(V_interpolated[j][tmp_i, :], V[j].numpy()[250+tmp_i, :n_modes], atol=1e-6))
            print()

        res0 = 3*[np.zeros((n_nodes, snapshots_per_dataset))]
    ...
    for ts in [0, 15]:
        ...
        # u_orig = U_u[:, i_orig, ts]
        # v_orig = U_v[:, i_orig, ts]
        # p_orig = U_p[:, i_orig, ts]
        # fig, ax = plot_up(u_orig, v_orig, p_orig, x, y, tri)
        # plt.title("FOM")
        # plt.show()

        fig, ax = plot_up(u_int2, v_int2, p_int2, x, y, tri)
        plt.title("ROM without test data")
        plt.show()
