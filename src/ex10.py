...
def project_STS_eigenvectors_to_S_eigenvectors(V, X):
    """
    X = U S VT
    U = X V / sqrt(S)
    here: only U = X V is happening
    """
    sizes = X.getSizes()[0]
    N = V.getActiveColumns()[1]
    U = SLEPc.BV().create(PETSc.COMM_SELF)
    U.setSizes(sizes, N)
    U.setActiveColumns(0, N)
    U.setFromOptions()

    tmpvec2 = X.createVecs('left')
    for i in range(N):
        tmpvec = V.getColumn(i)
        X.mult(tmpvec, tmpvec2)
        U.insertVec(i, tmpvec2)
        V.restoreColumn(i, tmpvec)
...
def project_reduced_to_full_space(alpha, bv):
    ...
    scatter.begin(alpha, Wr, PETSc.InsertMode.INSERT,
